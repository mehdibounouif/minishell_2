...
/*   Updated: 2025/08/13 01:39:55 by mbounoui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mini_shell(t_mini *minishell, t_env *envp)
{
	while (1)
	{
		handle_signal();
		...
		if (!open_herdocs(minishell->tree, envp))
			continue ;
		sig_ctrl(1);
		execute_full_command(minishell->tree, &envp, 0);
		sig_ctrl(0);
	}
}
